...
get_xkb_option_from_name (const XkbModifier *modifier, const gchar* name)
{
  ...
  for (int i = 0; options[i].label && options[i].xkb_option; i++)
    {
                                       ...
                                       const gchar         *name)
{
  ...
  for (GSList *l = self->radio_group; l != NULL; l = l->next)
    {
...
update_active_radio (CcXkbModifierDialog *self)
{
  ...
  for (guint i = 0; options != NULL && options[i] != NULL; i++)
    {
                                         ...
                                         gpointer  user_data)
{
  ...
  for (guint i = 0; items != NULL && items[i] != NULL; i++)
    {
